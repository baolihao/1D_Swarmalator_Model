...
xij                  = repmat(x, N, 1) - repmat(x(:), 1, N); %(d*N) *N
rij                  = get_pairwise_dist(xij, d, N);  % N*N
% find the other pairwise quantities
switch ode_type
  case 0
    xiij             = xi(:) - xi; %N*N
  case 1
    thetaij          = theta(:) - theta;%N*N
    xiij             = xi(:) - xi; %N*N
% use the neighborhood
    ind              = rij <= r_neighbor;    
    N_sys            = repmat(sum(ind, 2), 1, N); %N*N
  case 2
    vij              = repmat(v, N, 1) - repmat(v(:), 1, N);
    sij              = get_pairwise_dist(vij,d,N);%%
    xiij             = xi(:) - xi;
end
...
switch ode_type
  ...
  case 1
    idx1             = 1;
    idx2             = d * N;
    v                = [cos(theta); sin(theta)]; %2*N
    rhs(idx1 : idx2) = v(:) + sum(kron(phi{1}(rij, thetaij, xiij)./N_sys, ones(d, 1)).*xij, 2);
    idx1             = d * N + 1;
    idx2             = (d + 1) * N;
    rhs(idx1 : idx2) = sum(phi{2}(rij, thetaij, xiij)./N_sys.*thetaij, 2); %% N_sys
    idx1             = (d + 1) * N + 1;
    idx2             = (d + 2) * N;
    rhs(idx1 : idx2) = sum(phi{3}(rij, thetaij, xiij)./N_sys.*xiij, 2);
  case 2
    idx1             = 1;
    idx2             = d * N;
    rhs(idx1 : idx2) = v(:);
